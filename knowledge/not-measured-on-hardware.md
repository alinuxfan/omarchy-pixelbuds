---
type: reference
title: This bundle was written without Pixel Buds Pro 2 on hand
...
Google's published Fast Pair specification, with no Pixel Buds Pro 2 attached
...
- `pixelbudsd` run with no paired device fails exactly the way the code
  ...
  (25e97ff7-24ce-4c4c-8951-f764a708f7b5); pair your Pixel Buds Pro 2 first, or
  pass --device <MAC>`.
- `pbp2ctl` run with no daemon listening fails exactly the way the code says
  it should: `could not reach pixelbudsd at $XDG_RUNTIME_DIR/pixelbudspro2.sock`.
...
- Whether the Maestro UUID advertisement and RFCOMM channel resolution behave
  identically on Pixel Buds Pro 2 as on the Pixel Buds Pro `pbpctrl` was
  written against; its own README hedges with "might or might not work on
  other Pixel Buds devices."
