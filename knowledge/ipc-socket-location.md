...
status: stable
verified:
  - by: reading pbp2-common::socket_path and its unit test, socket_path_refuses_to_fall_back
    at: 2026-08-17
---
...
start: `pbp2_common::socket_path()` returns
`$XDG_RUNTIME_DIR/pixelbudspro2.sock`, and `main.rs` sets its permissions to
...
mode and the same fix, just in Rust: `pbp2-common::socket_path()` and
`state_path()` are called by both `pixelbudsd` and `pbp2ctl`, and `parse_verb`
...
`Err` via `.context(...)` before touching the network at all), and `pbp2ctl`
...
`pbp2-common`'s `socket_path_refuses_to_fall_back` test asserts both halves:
